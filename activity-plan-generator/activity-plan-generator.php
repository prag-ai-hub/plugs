 ...
 * Description: A plugin to generate educational activities based on grade levels and topics with optional centralized Assistant API support.
 * Version: 2.0
...
// OpenAI API call function - IMPROVED VERSION WITH ASSISTANT API SUPPORT
function activity_openai_api_call($model, $prompt) {
    ...
    error_log('========== ACTIVITY GENERATOR: API METHOD DETECTION START ==========');
    error_log('Activity Generator: User ID: ' . $user_id);
    error_log('Activity Generator: Membership Level: ' . $level_name . ' (ID: ' . $level_id . ')');
    error_log('Activity Generator: Model: ' . $model);
    
    // Check if centralized plugin function exists
    $function_exists = function_exists('eduai_is_assistant_enabled');
    error_log('Activity Generator: eduai_is_assistant_enabled() function exists: ' . ($function_exists ? 'YES' : 'NO'));
    
    // Check if Assistant API is enabled
    $assistant_enabled = false;
    if ($function_exists) {
        $assistant_enabled = eduai_is_assistant_enabled();
        error_log('Activity Generator: Assistant API enabled status: ' . ($assistant_enabled ? 'ENABLED' : 'DISABLED'));
        
        // Get more details about the configuration
        if (function_exists('eduai_get_status')) {
            $status = eduai_get_status();
            error_log('Activity Generator: Assistant API Status Details: ' . print_r($status, true));
        }
    } else {
        error_log('Activity Generator: Centralized Assistant API Integration plugin is NOT active or not installed');
    }
    
    // Check if the centralized Assistant API plugin is available AND enabled
    if ($function_exists && $assistant_enabled) {
        
        // USE ASSISTANT API (NEW METHOD)============================================
        error_log('========== ACTIVITY GENERATOR: USING ASSISTANT API METHOD ==========');
        error_log('Activity Generator: Routing to centralized Assistant API integration');
        ...
        $api_options = array(
            ...
            'max_tokens' => 2500,
            'temperature' => 0.2,
            'system_message' => 'You are an expert educational activity designer who creates detailed, engaging, and age-appropriate learning activities.',
            'source_plugin' => 'Activity Generator'  
        );
        ...
        if (!$api_response['success']) {
            error_log('Activity Generator: Error message: ' . $api_response['error']);
            return new WP_Error('assistant_api_error', 'Assistant API Error: ' . $api_response['error']);
        }
        
        // Get the generated content
        $activity_content = $api_response['content'];
        $content_length = strlen($activity_content);
        ...
        if ($usage_limit !== null) {
            ...
            update_user_meta($user_id, 'APG_openai_api_usage', $new_usage);
            error_log('Activity Generator: Usage updated from ' . $current_usage . ' to ' . $new_usage);
        } else {
            error_log('Activity Generator: No usage limit - Enterprise plan (unlimited usage)');
        }

        // Clean up the content
        $content = trim($activity_content);
        ...
        error_log('========== ACTIVITY GENERATOR: ASSISTANT API METHOD COMPLETED SUCCESSFULLY ==========');
        ...
        error_log('========== ACTIVITY GENERATOR: USING DIRECT OPENAI API METHOD ==========');
        
        if (!$function_exists) {
            error_log('Activity Generator: Reason - Centralized plugin not available');
        } else {
            error_log('Activity Generator: Reason - Assistant API toggle is disabled in admin settings');
        }
        
        error_log('Activity Generator: Using traditional wp_remote_post() to OpenAI API');
        ...
        error_log('Activity Generator: Direct API request sent, waiting for response...');

        if (is_wp_error($response)) {
            error_log('Activity Generator: ❌ Direct API call FAILED - WP Error');
            error_log('Activity Generator: Error message: ' . $response->get_error_message());
        ...
        error_log('Activity Generator: Received HTTP response code: ' . $response_code);

        if ($response_code === 200 && isset($body['choices'][0]['message']['content'])) {
            $activity_content = $body['choices'][0]['message']['content'];
            $content_length = strlen($activity_content);
            
            error_log('Activity Generator: ✅ Direct API call SUCCESSFUL');
            error_log('Activity Generator: Generated content length: ' . $content_length . ' characters');
            error_log('Activity Generator: Content preview (first 100 chars): ' . substr($activity_content, 0, 100) . '...');
            
            // Update the usage counter only after a successful response
            if ($usage_limit !== null) {
                error_log('Activity Generator: Updating usage counter for level ' . $level_id);
            ...
            error_log('Activity Generator: Content cleaned and ready to return');
            error_log('========== ACTIVITY GENERATOR: DIRECT API METHOD COMPLETED SUCCESSFULLY ==========');
            
            // Return clean HTML
            return $content;
        } else {
            error_log('Activity Generator: ❌ Direct API response INVALID');
            error_log('Activity Generator: Response code: ' . $response_code);
            error_log('Activity Generator: Response body: ' . print_r($body, true));
